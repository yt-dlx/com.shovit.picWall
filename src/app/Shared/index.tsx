// client/src/app/Shared/index.tsx
/* ============================================================================================================================== */
/* ============================================================================================================================== */
...
const CardContainer: React.FC<{ children: React.ReactNode; style?: any }> = ({ children, style }) => (
  <View style={[{ borderRadius: wp("5%"), backgroundColor: colorize("#171717", 1.0), shadowColor: "#000", shadowOffset: { width: 0, height: hp("0.5%") }, shadowOpacity: 0.3, shadowRadius: wp("2%"), elevation: 5, padding: wp("6%") }, style]}>
    {children}
  </View>
);
/* ============================================================================================================================== */
...
export default function SharedPage(): JSX.Element {
  ...
  useEffect(() => {
    Animated.loop(Animated.sequence([Animated.timing(opacity, { toValue: 1, duration: 1000, useNativeDriver: true }), Animated.timing(opacity, { toValue: 0, duration: 1000, useNativeDriver: true })])).start();
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      />
      <Animated.View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: wp("6%"), transform: [{ scale: scaleAnim }] }}>
        <CardContainer>
          {selectedImage && (
            <View
              style={{
          ...
          {adError ? (
            ...
            <View style={{ alignItems: "center", marginTop: hp("2%") }}>
              <Text style={{ color: colorize("#25BE8B", 1.0), fontSize: wp("5.5%"), fontFamily: "Lobster", fontWeight: "600", marginBottom: hp("2%") }}> Google Ad-Loading</Text>
              <Text style={{ color: colorize("#F4F4F5", 0.8), fontSize: wp("3.5%"), fontFamily: "Markazi", textAlign: "center", lineHeight: hp("2.5%") }}> After watching the ad, you'll be redirected to your selected content. </Text>
              <Text style={{ color: colorize("#F4F4F5", 0.9), fontSize: wp("4%"), fontFamily: "Markazi", marginTop: hp("3%") }}> Redirecting in {countdown} seconds... </Text>
            </View>
          )}
        </CardContainer>
      </Animated.View>
