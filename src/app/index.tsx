...
const UpdateDialog: FC<{ serverVersion: string; currentVersion: string }> = memo(({ currentVersion, serverVersion }) => {
  ...
  return (
    <View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, justifyContent: "center", alignItems: "center", padding: 16, zIndex: 1000 }}>
      <Animated.View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: colorize("#1B1C1D", 1.0) }} />
      <View style={{ borderRadius: 9999, padding: 4, backgroundColor: colorize("#1B1C1D", 0.8), justifyContent: "center", alignItems: "center" }}>
        <Image
          cachePolicy="disk"
          contentFit="contain"
      ...
      <View style={{ alignItems: "center" }}>
        ...
        >
          <Text style={{ fontSize: 20, fontFamily: "Kurale", color: colorize("#1B1C1D", 1.0) }}> Update Now </Text>
        </TouchableOpacity>
      </View>
    </View>
  );
    ...
    <View style={{ flex: 1, backgroundColor: colorize("#1B1C1D", 1.0) }}>
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", position: "relative" }}>
        <View style={{ flexDirection: "row", height: "100%", overflow: "hidden", position: "relative" }}>
          ...
          <LinearGradient
            colors={[colorize("#1B1C1D", 1.0), colorize("#1B1C1D", 0.4), colorize("#1B1C1D", 0.1), colorize("#1B1C1D", 0.4), colorize("#1B1C1D", 1.0)]}
            ...
            <Animated.View entering={FadeInDown.delay(600).duration(1500).springify()}>
              <View>
                ...
                <Animated.View style={{ alignSelf: "center" }} entering={FadeInDown.delay(600).duration(1500).springify()}>
                  <View style={{ borderRadius: 9999, paddingHorizontal: 12, paddingVertical: 4, backgroundColor: colorize("#1B1C1D", 0.9) }}>
              ...
              <Link href="./Home" asChild>
                ...
                >
                  <View style={[{ shadowColor: colorize("#1B1C1D", 1.0), shadowOffset: { width: 0, height: 4 } }]}>
                    <View style={{ paddingVertical: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: colorize("#FFFFFF", 1.0) }}>
                      <FontAwesome5 name="camera-retro" size={32} color={colorize("#1B1C1D", 1.0)} />
                      <Text style={{ fontSize: 35, fontFamily: "Jersey", color: colorize("#1B1C1D", 1.0) }}> Let&apos;s Explore </Text>
                    </View>
                  </View>
                </TouchableOpacity>
              </Link>
