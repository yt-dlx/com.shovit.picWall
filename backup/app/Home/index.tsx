      ...
      <View style={{ flex: 1, justifyContent: "flex-end" }}>
        <View style={{ height: "100%" }}>
          ...
          <View style={{ flex: 1, backgroundColor: colorize("#111111", 0.6), overflow: "hidden" }}>
            <View
              style={{ padding: 20, flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: colorize("#111111", 0.6) }}
            >
              <Text style={{ fontFamily: "Lobster", fontSize: 40, textDecorationLine: "underline", color: colorize("#F4F4F5", 1.0) }}>
                Categories + Styles
              </Text>
            </View>
...
const SearchBar: FC<{ onSearch: (text: string) => void }> = memo(({ onSearch }) => {
  ...
  return (
    <View style={{ marginTop: 5 }}>
      ...
      >
        <FontAwesome5 name="search" size={16} color={colorize("#F4F4F5", 0.8)} />
        <TextInput
          value={searchText}
          ...
          style={{ flex: 1, marginLeft: 8, fontSize: 16, fontFamily: "Markazi", color: colorize("#F4F4F5", 1.0) }}
        />
        {searchText.length > 0 && (
          <TouchableOpacity onPress={() => handleSearch("")} style={{ padding: 8 }} accessibilityLabel="Clear search">
            <FontAwesome5 name="times" size={16} color={colorize("#F4F4F5", 0.8)} />
          </TouchableOpacity>
        )}
      </View>
    </View>
    ...
    {images.data.map((image, index) => {
      ...
      return (
        <Link
          ...
          href={{
            pathname: "/Shared",
            params: { data: JSON.stringify({ environment_title: images.environment_title, selectedIndex: fullDataIndex, data: images.allData }) }
          }}
          asChild
        >
          <TouchableOpacity
  ...
  return (
    ...
    >
      <Link
        href={{
          pathname: "/Shared",
          params: { data: JSON.stringify({ environment_title: data.environment_title, selectedIndex: currentIndex, data: data.images }) }
        }}
        asChild
      >
        <TouchableOpacity accessibilityLabel="View full wallpaper">
          <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
            <View style={{ position: "relative", width: "100%", height: 500, overflow: "hidden" }}>
              ...
              />
              {loading && (
                <View style={[StyleSheet.absoluteFill, { backgroundColor: colorize("#111111", 0.2) }]} className="flex items-center justify-center">
              ...
              <View style={{ position: "absolute", bottom: -2, left: 0, right: 0 }}>
                ...
                <View style={{ padding: 4, margin: 4 }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
                    <Text style={{ flex: 1, fontSize: 14, marginRight: 8, color: colorize("#F4F4F5", 1.0), fontFamily: "Markazi" }}>
                      {data.images[currentIndex].original_file_name.replace(/_/g, " ").replace(".jpg", "")}
                    </Text>
                    <View style={{ paddingHorizontal: 6, paddingVertical: 2 }}>
                      <Text style={{ fontSize: 12, color: colorize("#F4F4F5", 1.0), fontFamily: "Markazi" }}>
          ...
          <View style={{ position: "absolute", top: "35%", right: 8, padding: 4, transform: [{ translateY: -70 }], borderRadius: 15 }}>
            <View style={[StyleSheet.absoluteFillObject, { borderRadius: 15, overflow: "hidden", backgroundColor: colorize("#111111", 0.8) }]} />
            {data.images.slice(0, 3).map((img, idx) => (
              <TouchableOpacity
                key={idx}
                onPress={() => animateTransition(idx)}
                style={{ marginBottom: idx < 2 ? 2 : 0 }}
...
const CategoryButton: FC<CategoryButtonExtendedProps> = memo(({ category, onPress, rawCategoriesArray, count }) => {
  ...
  return (
    <TouchableOpacity
      onPress={() => onPress()}
      style={{ flex: 1, height: 60, margin: 4, borderRadius: 15, overflow: "hidden", minWidth: 120 }}
      ...
      <View style={{ borderRadius: 15, overflow: "hidden", width: "100%", height: "100%" }}>
        ...
        >
          <MaterialCommunityIcons
            ...
            style={{ marginRight: 4 }}
          />
          <Text style={{ fontFamily: "Lobster", color: colorize("#F4F4F5", 1.0), fontSize: 18, textAlign: "center" }}>
    ...
    <Animated.View style={fadeInStyle}>
      ...
      <View style={{ marginTop: 20, paddingRight: 6, paddingLeft: 6, paddingBottom: 10 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
          ...
          <Text style={{ fontFamily: "Lobster", fontSize: 20, color: colorize("#F4F4F5", 1.0), textAlign: "center", marginHorizontal: 10 }}>
...
export default function HomePage(): JSX.Element {
    ...
    if (searchQuery) {
      return (
        <View style={{ padding: 20, alignItems: "center" }}>
          <Text style={{ fontFamily: "Markazi", color: colorize("#F4F4F5", 1.0), fontSize: 16, textAlign: "center" }}>
  ...
  return (
    <View style={{ backgroundColor: colorize("#111111", 1.0), flex: 1, position: "relative" }}>
